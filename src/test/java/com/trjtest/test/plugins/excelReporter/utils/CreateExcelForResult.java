package com.trjtest.test.plugins.excelReporter.utils;

import com.trjtest.test.pagesfunction.GetExcelData;
...
public class CreateExcelForResult {
     ...
     * excel文件读取说明
     **/
    static File fileFromExcel = null;
    /**
     * 用于写入excel的文件流
     */
        ...
        try {
            ...
            for (ISuite suite : suites) {
                ...
                for (ISuiteResult r : tests.values()) {//这里循环的是每个tests
                    ...
                    /**获取该模块中用例的列数**/
                    int col = GetExcelData.getNum(overview.getName(), "col");
                    //合并单元格
                    funtions.addMergedRegion(new CellRangeAddress(0, 0, 0, col - 1));
                    //创建一个cell
                    ...
                    setRangeCellBorder(new CellRangeAddress(0, 0, 0, col - 1), workbook.getSheet(overview.getName()), workbook);

                    int row = GetExcelData.getNum(overview.getName(), "row");
                    for (int i = 0; i < row; i++) {

                        if (i == 0) {
                            XSSFRow rownum = funtions.createRow(i + 1);
                            String labelsForFunctions[] = GetExcelData.getRowValues(overview.getName(), i);
                            for (int j = 0; j < labelsForFunctions.length; j++) {
                                rownum.createCell(j).setCellValue(new XSSFRichTextString(labelsForFunctions[j]));
                                rownum.getCell(j).setCellStyle(setCellStyle3(workbook));
                            }
                        } else {
                            XSSFRow rownum = funtions.createRow(i + 1);
                            String labelsForFunctions[] = GetExcelData.getRowValues(overview.getName(), i);
                            ...
                            for (int j = 0; j < labelsForFunctions.length; j++) {
                                ...
                                if (j == labelsForFunctions.length - 1) {
                                    ...
                                    rownum.createCell(j).setCellValue(new XSSFRichTextString(labelsForFunctions[j]));
                                    rownum.getCell(j).setCellStyle(setContentCellBorder(workbook));
                ...
                for (int i = 0; i < testNum; i++) {
                    ...
                    for (int j = 0; j < 6; j++) {
                        ...
                        if (j == 2) {
                            //用例总数
                            cell.setCellValue(GetExcelData.getTestsNum(new XSSFRichTextString(workbook.getSheetName(i + 1)), "All"));
                        ...
                        if (j == 3) {
                            //成功条数
                            cell.setCellValue(GetExcelData.getTestsNum(new XSSFRichTextString(workbook.getSheetName(i + 1)), "Pass"));
                        ...
                        if (j == 4) {
                            ...
                            cell.setCellValue(GetExcelData.getTestsNum(new XSSFRichTextString(workbook.getSheetName(i + 1)), "Fail"));
                            cell.setCellStyle(setContentCellBorder(workbook));
                        }

                        if (j == 5) {
                            //通过率
                            cell.setCellValue(getPercent(GetExcelData.getTestsNum(new XSSFRichTextString(workbook.getSheetName(i + 1)), "Pass"), GetExcelData.getTestsNum(new XSSFRichTextString(workbook.getSheetName(i + 1)), "All")));
                            cell.setCellStyle(setContentCellBorder(workbook));
                        }
                    }
                }
...
//                            String functionPack = caseName.substring(0, caseName.indexOf("_"));
//                            //设置用例名字
...
//                            //添加截图
//                            File png = new File("result/screenshots/" + caseName + ".png");
//                            if (png.exists()) {
//                                //设置截图图片，这是超链接
...
//                                        ("HYPERLINK(\"" + new File("result/screenshots/" + caseName + ".png").getAbsolutePath() + "\",\"" + caseName + ".png\")");
//                            } else {
...
//                        XSSFCellStyle xcs = workbook.createCellStyle();
//
//                        List<ITestNGMethod> iTestNGMethodList = new ArrayList<>(overview.getPassedTests().getAllMethods());
//                        synchronized (iTestNGMethodList) {
//                            for (int i = 0; i < iTestNGMethodList.size(); i++) {
//                                ITestNGMethod im = iTestNGMethodList.get(i);
//                                String caseName = im.getTestClass().getRealClass().getSimpleName();
//                                String functionPack = caseName.substring(0, caseName.indexOf("_"));
...
//                                        ("HYPERLINK(\"" + ("file://" + new File("result/logs/" + functionPack + "/" + caseName + ".log").getAbsolutePath()) + "\",\"" + caseName + ".log\")");
//                                xcs.setFillForegroundColor((short) 57);// 设置背景色 -绿色
//                                setCellBorder(workbook, xcs);
