...
public class SelectDriver {
    /**
     *  根据测试平台的不同生成不同的　driver，比如AndroidDriver或者IOSDriver
     **/
    //声明driver
    ...
    public String nativeWebTap;
    public static Logger logger = Logger.getLogger(SelectDriver.class);

    public AppiumDriver selectDriver(ITestContext context, AppiumUtil appiumUtil, Map<String, String> modifyDriver) {
        //通过testng的xml文件获取serverURL参数值，并赋给  serverURL变量http://127.0.0.1:4444/wd/hub
        serverURL = context.getCurrentXmlTest().getParameter("serverURL") + modifyDriver.get("port") + "/wd/hub";
        ...
        platformVersion = modifyDriver.get("platformVersion");
        //通过testng的xml文件获取deviceName参数值，并赋给  deviceName变量
        deviceName = modifyDriver.get("deviceName");
        ...
        //通过testng的xml文件获取iOS设置的UDID并赋值给udid
        udid = modifyDriver.get("udid");
        nativeWebTap = context.getCurrentXmlTest().getParameter("nativeWebTap");
